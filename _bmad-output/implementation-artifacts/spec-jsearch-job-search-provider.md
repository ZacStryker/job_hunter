...
- **2026-07-18 — Coverage-spike findings (free-tier, real API).** Ran the broad spike + a focused NL/EU spike (`spike/jsearch-coverage-nl.ts`) + raw country probes. **Verdict: go**, with two documented caveats. Strong, uniform coverage (10 results/query, 100% apply links, ~9–10 distinct employers) for **US, Canada, Netherlands, Germany, France** — identical whether or not a `city` is supplied, so the earlier "does `country=nl` need a `city`?" theory is dead (those NL zeros were the pre-v2 location-in-query bug, already fixed). **Caveat 1: UK (`gb`), Ireland (`ie`), and Australia (`au`) return zero inventory** on this tier — `200 OK`, empty `data.jobs`, param echoed back in `request.parameters`, so it is a vendor-side data gap, not our bug and not fixable by param tweaks (tried `gb`/`uk`/`GB`/`ie`). **Caveat 2: salary is ~0% populated outside the US.** Also noted: Canadian/some-EU results carry a null `job_city` (location resolves to country only). All three caveats logged in `deferred-work.md` for the ingest/UI story to handle as graceful degradation. No provider code change — the seam and `normalize()` are correct.
