...
- **JSearch currently returns ZERO inventory for UK (`gb`), Ireland (`ie`), and Australia (`au`)** — a live coverage hole, not a bug on our side. Measured 2026-07-18 against the real API on the Basic tier: `us`, `ca`, `nl`, `de`, `fr` all return a full page of 10 with 100% apply links (with *or* without a `city`); `gb`/`uk`/`GB`/`ie`/`au` all return `200 OK` with `data.jobs: []` while echoing the country param back in `request.parameters`. So the param is well-formed and accepted — the backend simply has no data for those markets on this tier. The ingest/UI story must (a) NOT surface UK/IE/AU as "supported" markets without a live re-check, and (b) treat an empty result set as "no coverage here", not an error. Re-probe before any user targets those regions; if a paid tier restores them, this is purely a data-availability change with no code impact. [`spike/jsearch-coverage-nl.ts`, raw probes]
- **Salary data is essentially absent outside the US** (0% fill across all NL/DE/FR/CA queries measured 2026-07-18; sparse even in the US). `normalize()` already yields `salary: null` when both bounds are missing, so no correctness issue — but any salary-based sort, filter, or UI badge in the ingest/search story must degrade gracefully to "not disclosed" rather than assuming the field is populated. [`jsearch-provider.ts` `formatSalary`]
- **Canadian (and some EU) results carry a null `job_city`** — geo resolves only to the country (`job_country: 'CA'`), so `formatLocation()` yields `"CA"` rather than `"Toronto, CA"` even for a city-scoped query. Cosmetic: the search UI should not promise city-level location text for every result. [`jsearch-provider.ts` `formatLocation`]

## From spec-jsearch-discovery-wiring (2026-07-18)

- **The JSearch provider `fetch` has no timeout, and discovery now awaits it.** `jsearch-provider.ts`'s `fetch(JSEARCH_URL, …)` carries no `AbortSignal.timeout(...)`, unlike the scraper fetch in `discovery-service.ts` which uses `AbortSignal.timeout(120_000)`. Now that `runDiscovery` calls the provider inside `Promise.allSettled`, a hung RapidAPI request hangs the whole discovery run — the webhook stream never completes and the `activityRegistry` run never finalizes, which is exactly the "hung run blocks new runs of that type until restart" failure already logged for discovery/analysis (2026-06-26). Pre-existing in the shipped provider; wiring makes it reachable. Fix: add `AbortSignal.timeout(...)` to the provider fetch (mirrors the scraper), so a stall surfaces as a per-source error instead of a hang. [`job-hunt-dashboard/src/server/services/job-search/jsearch-provider.ts`]
- **Scheduled/repeated discovery burns the shared paid RapidAPI quota with no guard.** Each `runDiscovery` invocation calls `provider.search()` once per enabled `jsearch` search config — one billable RapidAPI call each, against the single shared `JSEARCH_API_KEY` (HITLOBSTER pays for all tenants). Nothing rate-limits or budget-caps this; a busy multi-tenant instance, or a cron-driven discovery loop, can exhaust the plan's call quota. Not a code defect — inherent to the shared-key design the human chose — but worth an operational guard (per-tenant/day cap, or a global monthly budget check before calling) before enabling jsearch broadly. [`job-hunt-dashboard/src/server/services/discovery-service.ts` jsearch branch]
